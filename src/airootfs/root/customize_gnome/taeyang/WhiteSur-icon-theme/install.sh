...
install() {
  ...
  mkdir -p                                                                                   "${THEME_DIR}"
  mv "${SRC_DIR}"/{COPYING,AUTHORS}                                                       "${THEME_DIR}"
  mv "${SRC_DIR}"/src/index.theme                                                         "${THEME_DIR}"
  ...
  if [[ ${color} == '' ]]; then
    ...
    mv "${SRC_DIR}"/src/{actions,animations,apps,categories,devices,emotes,emblems,mimes,places,preferences} "${THEME_DIR}"
    mv "${SRC_DIR}"/src/status/{16,22,24,32,symbolic}                                     "${THEME_DIR}"/status
    ...
    if [[ ${bold:-} == 'true' ]]; then
      mv "${SRC_DIR}"/bold/*                                                              "${THEME_DIR}"
    fi

    if [[ $DESKTOP_SESSION == '/usr/share/xsessions/budgie-desktop' ]]; then
      mv "${SRC_DIR}"/src/status/symbolic-budgie/*.svg                                    "${THEME_DIR}"/status/symbolic
    fi

    if [[ ${alternative:-} == 'true' ]]; then
      mv "${SRC_DIR}"/alternative/*                                                       "${THEME_DIR}"
    fi

    if [[ ${theme} != '' ]]; then
      mv "${SRC_DIR}"/colors/color${theme}/*.svg                                          "${THEME_DIR}"/places/scalable
    fi

    rm -rf "${THEME_DIR}"/places/scalable/user-trash{'','-full'}-dark.svg

    mv "${SRC_DIR}"/links/{actions,apps,categories,devices,emotes,emblems,mimes,places,status,preferences} "${THEME_DIR}"
    ln -s "${THEME_DIR}"/preferences/32 "${THEME_DIR}"/preferences/22
  fi

  if [[ ${color} == '-light' ]]; then
    mkdir -p                                                                                 "${THEME_DIR}"/status
    mv ${SRC_DIR}/src/status/{16,22,24,32}                                                "${THEME_DIR}"/status

    if [[ ${bold:-} == 'true' ]]; then
      mv "${SRC_DIR}"/bold/status/{16,22,24}                                              "${THEME_DIR}"/status
    fi

    # Change icon color for light theme
    sed -i "s/#f2f2f2/#363636/g" "${THEME_DIR}"/status/{16,22,24,32}/*
    mv "${SRC_DIR}"/links/status/{16,22,24,32}                                            "${THEME_DIR}"/status
  ...
  if [[ ${color} == '-dark' ]]; then
    ...
    mv "${SRC_DIR}"/src/actions                                                           "${THEME_DIR}"
    mv "${SRC_DIR}"/src/apps/{22,32,symbolic}                                             "${THEME_DIR}"/apps
    mv "${SRC_DIR}"/src/categories/{22,symbolic}                                          "${THEME_DIR}"/categories
    mv "${SRC_DIR}"/src/emblems/symbolic                                                  "${THEME_DIR}"/emblems
    mv "${SRC_DIR}"/src/mimes/symbolic                                                    "${THEME_DIR}"/mimes
    mv "${SRC_DIR}"/src/devices/{16,22,24,32,symbolic}                                    "${THEME_DIR}"/devices
    mv "${SRC_DIR}"/src/places/{16,22,24,scalable,symbolic}                               "${THEME_DIR}"/places
    mv "${SRC_DIR}"/src/status/symbolic                                                   "${THEME_DIR}"/status

    if [[ ${bold:-} == 'true' ]]; then
      mv "${SRC_DIR}"/bold/actions/symbolic/*.svg                                         "${THEME_DIR}"/actions/symbolic
      mv "${SRC_DIR}"/bold/apps/symbolic/*.svg                                            "${THEME_DIR}"/apps/symbolic
      mv "${SRC_DIR}"/bold/devices/symbolic/*.svg                                         "${THEME_DIR}"/devices/symbolic
      mv "${SRC_DIR}"/bold/status/symbolic/*.svg                                          "${THEME_DIR}"/status/symbolic
    fi

    if [[ ${alternative:-} == 'true' ]]; then
      mv "${SRC_DIR}"/alternative/apps/symbolic/*.svg                                     "${THEME_DIR}"/apps/symbolic
      mv "${SRC_DIR}"/alternative/places/scalable/*.svg                                   "${THEME_DIR}"/places/scalable
    fi
    ...
    mv "${SRC_DIR}"/links/actions/{16,22,24,32,symbolic}                                  "${THEME_DIR}"/actions
    mv "${SRC_DIR}"/links/devices/{16,22,24,32,symbolic}                                  "${THEME_DIR}"/devices
    mv "${SRC_DIR}"/links/places/{16,22,24,scalable,symbolic}                             "${THEME_DIR}"/places
    mv "${SRC_DIR}"/links/apps/{22,symbolic}                                              "${THEME_DIR}"/apps
    mv "${SRC_DIR}"/links/categories/{22,symbolic}                                        "${THEME_DIR}"/categories
    mv "${SRC_DIR}"/links/mimes/symbolic                                                  "${THEME_DIR}"/mimes
    mv "${SRC_DIR}"/links/status/symbolic                                                 "${THEME_DIR}"/status
